...
try{ $rs_juego= $db->Execute("SELECT cod_juego AS codigo,  descripcion 
                				from kaizen.juego order by 2");
}catch(exception $e){die($db->ErrorMsg());}


$_pagi_sql ="SELECT rs.id_localidad AS id,
...
<br/>
<div id="titulo_ventana" align="center"> EXCEPCION DE IMPUESTOS MUNICIPALES POR JUEGOS - 
REGIMEN SIMPLIFICADO</div>
<br/>

 <form action="#" method="post" enctype="multipart/form-data" name="formulario" id="formulario" onSubmit="ajax_post('contenido','municipal/abm_excepcion_mun_juego_rs.php',this); return false;">
	
<table border="0" width="80%" cellpadding="2" cellspacing="0" align="center">
	<th colspan="9" align="center" class="titulosgrandes"   ></th>
    	<tr class="td5">
   	 		 <td>Provincia</td>
	 		 <td><?php armar_combo_seleccione_ejecutar_ajax_post($rs_prov,"id_provincia",$id_provincia,'contenido','municipal/abm_excepcion_mun_juego_rs.php','formulario')?> </td>
             <td>Localidad</td>         
	 		 <td><?php armar_combo_seleccione($rs_localidad,"id_localidad",$id_localidad);?></td>
     		 <td>Juegos</td>
	  		 <td><?php armar_combo_todos($rs_juego,"id_juego",$id_juego);?></td>
	 		 <td><input type="submit" alt="buscar" value="Buscar" name="btnbuscar"   width="20" height="20"></td>
             <td width="221"  align="center" valign="middle">
    			<a href="#" onclick="window.open('municipal/listado_excep_municipal_juego_rs.php', '_blank', 'height=480, width=640, left=0, top=0, toolbar=no, menubar=no, titlebar=no, resizable=yes, scrollbars=yes')">
    			<img src="image/Adobe Acrobat Distiller 7.png" width="25" height="25" border="0" /></a></td>
		</tr>
    <th colspan="9" align="center" class="titulosgrandes" ></th>
</table>
</form>
...
<table border="0" width="35%" cellpadding="2" cellspacing="0" align="center"  >
<tr>
	<td align="center">
		<a href="#" onclick="ajax_get('contenido','municipal/alta_excepcion_mun_juego_rs.php','');"> 
		<span class="texto3"> Nueva Excepcion - Regimen Simplificado</span>
		<img src="image/New File.png" alt="Regresar" wid td="16" height="16" border="0"/></a>
	</td>
</tr>
</table>
...
<table width="56%" border="0" align="center">
	...
	<?php while ($row_resul = $_pagi_result->FetchNextObject($toupper=true)){?>
		<tr>
			...
			<td  align="center"><a href="#" onClick="ajax_get('contenido','municipal/modificar_excep_mun_juego_rs.php','id_localidad=<?php echo $row_resul->ID;?>&id_juego=<?php echo $row_resul->ID_JUEGO;?>')"><img border="0" alt="Editar" src="image/b_edit.png" width="16" height="16"> </a></td> 
		</tr>
	<?php } ?>
</table>
</div>