...
$_pagi_result2 =    try {
						$rs = $db_jue->Execute("$_pagi_sql");
					    }
						catch  (exception $e) 
						{ 
						die($db_jue->ErrorMsg());
	   					}
$_pagi_totalReg = $rs->RecordCount();//total de registros
...
// $_pagi_inicial = ($_pagi_actual-1) * $_pagi_cuantos;
 $_pagi_inicial = ($_pagi_actual*$_pagi_cuantos)-($_pagi_cuantos-1); 
 
 // Consulta SQL. Devuelve $cantidad registros empezando desde $_pagi_inicial
 $_pagi_final  = $_pagi_inicial+$_pagi_cuantos-1;
 $_pagi_sqlLim = "SELECT * FROM ($_pagi_sql) WHERE fila between $_pagi_inicial and $_pagi_final";
 try {
		$rs = $db_jue->SelectLimit("select * from emp where empno=>:emp order by empno",10,1, array('emp'=>7900))Execute("$_pagi_sql");
		}
		catch  (exception $e) 
		{ 
		die($db_jue->ErrorMsg());
...
// $_pagi_result = sql($link,$_pagi_sqlLim);
...
// $_pagi_desde = $_pagi_inicial + 1;
 $_pagi_desde = $_pagi_inicial;
 
 // Número del último registro de la página actual
 $_pagi_hasta = $_pagi_inicial + $_pagi_cuantos - 1;
